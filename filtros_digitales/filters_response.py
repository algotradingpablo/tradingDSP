...
ComplexArray_512 = NDArray[Shape["512"], Complex]
FloatArray_512 = NDArray[Shape["512"], Float]
...
class Filter:
    ...
    complex_amplitude: ComplexArray_512 = field(init=False, repr=False)
    w: FloatArray_512 = field(init=False, repr=False)
    gd: FloatArray_512 = field(init=False, repr=False)
    frequency: FloatArray_512 = field(init=False, repr=False)
    period: FloatArray_512 = field(init=False, repr=False)
    dbpowergain: FloatArray_512 = field(init=False, repr=False)
    phase: FloatArray_512 = field(init=False, repr=False)
    ...
    def plotNormFreqResponse(self) -> None:
        ...
        self.__plotFig(x = self.frequency, y = self.dbpowergain,
                       xlabel = 'Frequency (cycles/sample) [Nyquist = 0.5]', ylabel = 'Magnitude Gain (dB)',
    ...
    def plotPeriodResponse(self) -> None:
        ...
        self.__plotFig(x = self.period, y = self.dbpowergain, 
                       xlabel = 'Period (samples/cycle)', ylabel = 'Magnitude Gain (dB)', 
    ...
    def plotGroupDelay(self) -> None:
        ...
        self.__plotFig(x = self.frequency, y = self.gd,
                       xlabel = 'Frequency (cycles/sample) [Nyquist = 0.5]',
...
def TwoBarEMA(alpha: float = 0.2) -> Filter:   
...
def FourBarSMA() -> Filter:
    filt = Filter(name = "4-bar window SMA", 
...
def EhlersBandPass(pc: int = 20, fs: int = 1) -> Filter:
    pi = np.pi
    center_period = fs*pc
...
def EhlersSecondOrderHP(pc:int = 48, fs:int = 1, mult:float = np.sqrt(2)/2) -> Filter:
    pi = np.pi
    period = pc*fs
    alpha1 = (np.cos(mult*2*pi/period) + np.sin(mult*2*pi/period) - 1) / np.cos(mult*2*pi/period)
    c2 = (1 - alpha1)
    c3 = (1 - alpha1)**2
    c1 = (1 - alpha1 / 2)**2

    filt = Filter(name = "Ehlers High-pass filter", 
                  NumeratorZcoefs = [c1, -2*c1, c1],
                  DenominatorZcoefs = [1, -2*c2, c3],
                  fs = fs,
                  comments = "Second Order IIR")
    return filt
